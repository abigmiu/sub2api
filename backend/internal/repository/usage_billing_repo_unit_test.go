...
func TestDeductUsageBillingBalanceUsesSufficientBalanceGuard(t *testing.T) {
	...
	require.Equal(t, 7.5, newBalance)
	require.NoError(t, tx.Commit())
	require.NoError(t, mock.ExpectationsWereMet())
}

func TestDeductUsageBillingBalanceRecordsOverdraftWhenGuardMisses(t *testing.T) {
	ctx := context.Background()
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
...
func TestApplyUsageBillingEffectsFlagsBalanceOverdraft(t *testing.T) {
	ctx := context.Background()
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	require.Equal(t, -5.0, *result.NewBalance)
