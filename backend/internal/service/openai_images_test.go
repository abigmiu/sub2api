...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_AutoUsesUnstableRouting(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","size":"auto"}`)
	...
	require.Equal(t, ImageSizeRoutingUnstable, parsed.RoutingTier)
}
...
func TestOpenAIGatewayServiceForwardImages_APIKeyEmptyB64JSONNotBilled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","n":1,"response_format":"b64_json"}`)
	...
	svc := &OpenAIGatewayService{
		...
		httpUpstream: &httpUpstreamRecorder{
			resp: &http.Response{
				...
				Header: http.Header{
					"Content-Type": []string{"application/json"},
					"X-Request-Id": []string{"req_img_empty_b64"},
				},
				Body: io.NopCloser(strings.NewReader(`{"created":1710000010,"data":[{"b64_json":"","revised_prompt":"draw a cat"}]}`)),
			},
		},
	}
	parsed, err := svc.ParseOpenAIImagesRequest(c, body)
	...
	require.Equal(t, http.StatusOK, rec.Code)
	require.Equal(t, "", gjson.Get(rec.Body.String(), "data.0.b64_json").String())
}
